...
`https://github.com/EveryInc/compound-engineering-plugin` repository, which is
a real-world implementation of the conventions described above.

My project also follows the compound-engineering manifesto and conventions. Can
we create an AGENTS.md file for my project that follows the same conventions?
Please provide a template that I can fill in with my project's specific
details.

One small note: I really like using `tatr` for my tasks, so what I would change
is to keep my tasks in `./tasks` instead of `./todos`. Or at least `tatr` tasks
should be the ground truth for my tasks. The agent can write it's own `./todos`
for thinking and brainstorming, but the `tatr` tasks are the source of truth
